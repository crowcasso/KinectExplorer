...
{
    public class Slideshow : KinectGame
    {
        SpriteBatch spriteBatch;
        List<Texture2D> slides;
        TimeSpan SLIDE_LENGTH = new TimeSpan(0, 0, 20);
        TimeSpan currentSlideRuntime;
        int slideIndex = -1;
        TimeSpan TRANS_LENGTH = new TimeSpan(0, 0, 3);
        TimeSpan currentTrans;

        public Slideshow(GraphicsDeviceManager graphics, Size resolution) : base(graphics, resolution) { }
        ...
        public override void Update(Microsoft.Xna.Framework.GameTime gameTime)
        {
            ...
            if (currentSlideRuntime.CompareTo(SLIDE_LENGTH) > 0) nextSlide();
